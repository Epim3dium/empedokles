...
struct LooseTightDoubleGrid {
    ...
    int cellIndex(vec2f position) {
        int cell_x = std::clamp<int>(
                floor((position.x - bottom_left_corner.x) * inv_cell_size.x),
                0,
                num_cols - 1
        );
        int cell_y = std::clamp<int>(
                floor((position.y - bottom_left_corner.y) * inv_cell_size.y),
                0,
                num_rows - 1
        );
    ...
    AABB tightAABB(int index) {
        vec2f min = {
                (index % num_rows) * cell_size.x,
                (index / num_cols) * cell_size.y
        };
        return AABB::CreateMinMax(
                min + bottom_left_corner, min + cell_size + bottom_left_corner
        );
    }
    ...
    std::vector<Entity> query(AABB queried_area) {
        ...
        int minx = std::clamp<int>(
                floor((queried_area.min.x - bottom_left_corner.x) *
                      inv_cell_size.x),
                0,
                num_cols - 1
        );
        int maxx = std::clamp<int>(
                floor((queried_area.max.x - bottom_left_corner.x) *
                      inv_cell_size.x),
                0,
                num_cols - 1
        );
        int miny = std::clamp<int>(
                floor((queried_area.min.y - bottom_left_corner.y) *
                      inv_cell_size.y),
                0,
                num_rows - 1
        );
        int maxy = std::clamp<int>(
                floor((queried_area.max.y - bottom_left_corner.y) *
                      inv_cell_size.y),
                0,
                num_rows - 1
        );
        ...
        for (int y = miny; y <= maxy; y++) {
            ...
            for (int x = minx; x <= maxx; x++) {
                ...
                while (current_loose_node != -1) {
                    ...
                    int loose_cell_idx =
                            linked_loose_cells[current_loose_node].cell_idx;
                    ...
                    while (current_element != -1) {
                        ...
                        if (isOverlappingAABBAABB(
                                    linked_data[current_element].area,
                                    queried_area
                            )) {
                    ...
                    current_loose_node =
                            linked_loose_cells[current_loose_node].next;
                }
            }
        }
        return result;
    ...
    void insert(AABB object, Entity entity) {
        ...
        // linked_loose_cells.insert({tight_cells[cell_index].head,
        // cell_index});

        int minx = std::clamp<int>(
                floor((object.min.x - bottom_left_corner.x) * inv_cell_size.x),
                0,
                num_cols - 1
        );
        int maxx = std::clamp<int>(
                floor((object.max.x - bottom_left_corner.x) * inv_cell_size.x),
                0,
                num_cols - 1
        );
        int miny = std::clamp<int>(
                floor((object.min.y - bottom_left_corner.y) * inv_cell_size.y),
                0,
                num_rows - 1
        );
        int maxy = std::clamp<int>(
                floor((object.max.y - bottom_left_corner.y) * inv_cell_size.y),
                0,
                num_rows - 1
        );

        EMP_LOG_DEBUG << loose_cell.area.min.x << " " << loose_cell.area.min.y
                      << "\t" << loose_cell.area.max.x << " "
                      << loose_cell.area.max.y;
        for (int y = miny; y <= maxy; y++) {
            int trow = y * num_cols;
            for (int x = minx; x <= maxx; x++) {
                int index = trow + x;
                ...
                EMP_LOG_DEBUG << "\t" << area.min.x << " " << area.min.y << "\t"
                              << area.max.x << " " << area.max.y;
                if (isOverlappingAABBAABB(loose_cell.area, tightAABB(index))) {
                    tight_cell.head = linked_loose_cells.insert(
                            {tight_cell.head, loose_cell_index}
                    );
                }
            }
        }
    ...
    LooseTightDoubleGrid(AABB total_coverage, vec2f cell_size_)
        : cell_size(cell_size_),
          inv_cell_size(1.f / cell_size_.x, 1.f / cell_size_.y),
          bottom_left_corner(total_coverage.min),
          num_rows(total_coverage.size().y / cell_size.y + 1),
          num_cols(total_coverage.size().x / cell_size.x + 1),
          loose_cells(num_rows * num_cols, {-1, AABB::Expandable()}),
