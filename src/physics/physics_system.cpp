...
namespace emp {
    ...
    PhysicsSystem::PenetrationConstraint PhysicsSystem::m_handleCollision(Entity e1, const int convexIdx1, Entity e2, const int convexIdx2, float delT, float compliance) {
        ...
        auto& intersectingShape1 = col1.transformed_shape[convexIdx1];
        auto& intersectingShape2 = col2.transformed_shape[convexIdx2];
    ...
    std::vector<PhysicsSystem::PenetrationConstraint> PhysicsSystem::m_narrowPhase(const std::vector<CollidingPair>& pairs, float delT) {
        std::vector<PenetrationConstraint> result;
        for(const auto [b1, b2, s1i, s2i] : pairs) {
