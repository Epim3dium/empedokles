...
public:
    ...
    BasicRelativeString() : m_data(new Char[1]{'\0'}), m_length(0) {}
    ...
    BasicRelativeString(const BasicRelativeString& other) : m_data(new Char[other.m_length + 1]), m_length(other.m_length) {
        std::strcpy(m_data, other.m_data);
    }
    BasicRelativeString(BasicRelativeString&& other) noexcept : m_data(other.m_data), m_length(other.m_length) {
    ...
    bool operator==(const BasicRelativeString& other) const {
    ...
    const Char& front() const {
        if (m_length == 0) throw std::out_of_range("Front of empty string inaccessible");
        return *m_data;
    }
    const Char& back() const {
        if (m_length == 0) throw std::out_of_range("Back of empty string inaccessible");
    ...
    size_t find(iterable substr) const {
        size_t found_at = 0;
        size_t idx = 0;
    ...
    size_t find(const char* substr) const {
        return find(std::string(substr));
    }
    BasicRelativeString substr(size_t start, size_t size = npos) {
        if(start >= m_length) throw std::out_of_range("substr out of range");
...
namespace std {
template <>
struct hash<emp::RelativeString> {
