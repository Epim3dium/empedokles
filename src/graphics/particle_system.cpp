...
namespace emp {
ParticleSystem::ParticleSystem() : m_rndEngine(time(0)), m_rndDist(0, 1.f) {
...
void ParticleSystem::update(float delta_time) {
    for(auto e : entities) {
        ...
        if(emitter.enable_for_seconds > 0.f) {
            emitter.enable_for_seconds -=
                std::min(emitter.enable_for_seconds, delta_time);
        }
        ...
        m_queue.emit(count, emitter.type(), transform.position, emitter.type_data(), emitter.speed.range(), emitter.lifetime.range(), {0.f, EMP_PI * 2.f}, color);
    }
}
