...
template<typename T>
RelativePointer<T>::RelativePointer(T* raw) {
    set(raw);
}
template<typename T>
...
template<typename T>
RelativePointer<T>::operator T*() const {
    return get();
}
template<typename T>
bool RelativePointer<T>::is_null() const {
    return offset == 0;
}
template<typename T>
bool RelativePointer<T>::is_not_null() const {
    return !is_null();
}
template<typename T>
void RelativePointer<T>::set_null() {
    offset = 0;
}
template<typename T>
void RelativePointer<T>::set(void* raw) {
    if(raw == nullptr) {
        offset = 0;
        return;
    }
