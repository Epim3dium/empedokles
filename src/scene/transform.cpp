...
namespace emp {
    void Transform2D::m_updateLocalTransform() {
    ...
    void Transform2D::m_syncWithChange() {
    ...
    void Transform2D::setPositionNow(vec2f p) {
        position = p;
        m_syncWithChange();
    }
    void Transform2D::setRotationNow(float r) {
        rotation = r;
        m_syncWithChange();
    }
    void Transform2D::setScaleNow(vec2f s) {
        scale = s;
        m_syncWithChange();
    }
    void TransformSystem::update() {
        for(auto entity : entities) {
            auto& trans = coordinator.getComponent<Transform2D>(entity);
