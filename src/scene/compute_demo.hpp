    ...
    uint32_t layerCount) 
{
    ...
    void initializeImages(Device& device) {
        ...
        for(int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            display_image[i] = std::make_unique<TextureAsset>(
                device,
                VK_FORMAT_R8G8B8A8_SRGB,
                extent,
    ...
    void initializeDataBuffer(Device& device) {
        ...
        for(int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            // Create buffer
            dataBuffer[i] = std::make_unique<Buffer>(device,
                sizeof(uint32_t),
                dataCount,
    ...
    void performCompute(FrameInfo frame_info) {
        ...
        {
                display_image[frame_index]->transitionLayout(command_buffer,
                    VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
                output_image[frame_index]->transitionLayout(command_buffer,
                    VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
        }
        copyImageToImage(command_buffer,
            output_image[frame_index]->getImage(), display_image[frame_index]->getImage(), 256, 256, 1);
        {
            display_image[frame_index]->transitionLayout(command_buffer,
